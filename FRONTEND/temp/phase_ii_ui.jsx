...
`;
  ...
  const deleteWorkspace = async (id) => {
    ...
    try {
      ...
      // fetchWorkspaces();allways fetch workspace after deletion
    } catch (err) {
      console.log(err);
    }
  };
    ...
    try {
      ...
      if (responseData.data && responseData.data.createWorkspace) {
        const newWorkspace = {
          ...
          memberCount: responseData.data.createWorkspace.member
            ? responseData.data.createWorkspace.member.length
            : 0,
          groupCount: 0,
          color: "bg-indigo-500",
          groups: [],
  ...
  useEffect(() => {
    const fetchWorkspace = async (id = null, email = "tester@gmail.com") => {
      try {
        const token = localStorage.getItem("token");

        // Get current user's email from token or localStorage
        const userEmail = localStorage.getItem("userEmail"); // You'll need to store this during login
        // OR decode it from JWT token if you have that setup
        ...
        const response = await fetch("http://localhost:9999/graphql", {
          ...
          body: JSON.stringify({
            ...
            variables: {
              id: id,
              email: email || userEmail, // Use userEmail if no specific email provided
            },
          }),
        });
        ...
        if (responseData.data?.allWorkspaces) {
          setWorkspaces(
            responseData.data.allWorkspaces.map((ws) => ({
              ...ws,
    ...
    fetchWorkspace(); // This will now use the current user's email
  }, []);

  const openGROUP = (id) => {
    // localStorage.setItem("diskspaceid",id);
              ...
              {fields.map((field, index) => (
                <div key={field.id} className="flex gap-3">
                  <input
                    ...
                    className={`flex-1 p-3 bg-gray-50 text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Select a workspace to view groups and tasks
          </p>
        </div>
        ...
        {workspaces.map((workspace) => (
          ...
          >
            ...
            <div className="flex items-start justify-between mb-4">
              <div
                className={`w-12 h-12 ${
                  workspace.color || "bg-indigo-500"
                } rounded-lg flex items-center justify-center`}
              >
                <Folder className="w-6 h-6 text-white" />
              </div>
              <button onClick={() => openGROUP(workspace.id)}>
  ...
  //put this group view in another file and navigate when on click
